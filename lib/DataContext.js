...
// load stuff from the Tables. Set it up by providing a valid Oauth Token.
//
// Point is to hang all of the (pluralized) tables off of this as well as all
// of the services.
.class('DataContext').define({

  __constructor__: function (appToken) {

    this.accessToken = appToken
    const headers = {'Authorization': 'Bearer ' + this.accessToken}
    this.client = xmlrpc.createSecureClient({
      'url': 'https://api.infusionsoft.com/crm/xmlrpc/v1?access_token=' + this.accessToken,
      'headers': headers})
  ...
  _addService: function (iface) {
    ...
    _(typedef.signature(iface)).each(function (info, key) {
      ...
      hash[key] = function () {
        var args = [dataContext.accessToken].concat(_(arguments).toArray())
